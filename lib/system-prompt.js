...
- <head> MUST include the Tailwind CDN script — see "CSS — TAILWIND VIA CDN" below. Without it the page is unstyled.
- One JS in a single <script> before </body>. One <style> in <head> for the few things Tailwind can't do.

## CSS — TAILWIND VIA CDN (NON-NEGOTIABLE)
You MUST use Tailwind CSS via CDN. Without the CDN <script> tag in <head>, every Tailwind class in your output becomes a no-op and the page renders completely unstyled. This is the most common failure mode — do not skip it.

The first content inside <head> must follow this exact pattern (config first so the JIT compiler picks it up, CDN script second):
  ...
  <script>
    tailwind.config = {
      theme: {
        extend: {
          colors: {
            brand: '#3b82f6',    // ← choose your palette
            accent: '#f43f5e',
            surface: '#0a0a14',
          },
          fontFamily: {
...
Use Tailwind utility classes for ALL layout, spacing, color, typography, and responsive behavior. Reference your configured colors via Tailwind utilities (bg-brand, text-accent, bg-surface) — they only resolve if you've defined them in the tailwind.config above.

The <style> tag in <head> is for the few things Tailwind can't do, NOTHING ELSE:
- Custom @keyframes for animations
- .skip-link { … } and .skip-link:focus-visible { … }
- @media (prefers-reduced-motion: reduce) { … }
- @font-face only if not using Google Fonts CDN

Do NOT hand-write CSS that Tailwind already covers. If you find yourself writing .my-card { padding: 24px; … }, stop and use Tailwind utilities instead.
...
If either is missing, every Tailwind class in your output renders nothing. The page will look broken.`;
