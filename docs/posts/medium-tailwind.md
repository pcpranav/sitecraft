...
*Subtitle: Two weeks of prompt tuning didn't fix it. Switching to Tailwind via CDN did.*
...
This wasn't a one-model problem. I [tested four](https://pcpranavchandra.medium.com/i-tested-4-free-70b-class-llm-endpoints-for-real-production-work-heres-what-each-is-actually-a31aaeb10445) (Cerebras GPT-OSS 120B, Groq Llama 4 Scout, Cloudflare Qwen3 30B, OpenRouter free auto-router). All four showed the same pattern. The strongest model produced the most polished mobile experience and still had thin desktop.
...
> Asking an LLM to hand-write mobile-first responsive CSS is a bad task to ask an LLM.

It's brittle, repetitive, and the model has to make the same dozen decisions every time.
...
*More on the four providers I'm using: [I tested 4 free 70B-class LLM endpoints for real production work](https://pcpranavchandra.medium.com/i-tested-4-free-70b-class-llm-endpoints-for-real-production-work-heres-what-each-is-actually-a31aaeb10445).*
